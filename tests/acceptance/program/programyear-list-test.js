...
module('Acceptance | Program - ProgramYear List', function (hooks) {
  ...
  test('check list', async function (assert) {
    ...
    assert.equal(
      await getElementText('.programyear-list tbody tr:nth-of-type(1) td:nth-of-type(1)'),
      getText('2010 - 2011')
    );
    assert.equal(
      await getElementText('.programyear-list tbody tr:nth-of-type(1) td:nth-of-type(2)'),
      getText('cohort1')
    );
    assert.equal(
      await getElementText('.programyear-list tbody tr:nth-of-type(2) td:nth-of-type(1)'),
      getText('2011 - 2012')
    );
    assert.equal(
      await getElementText('.programyear-list tbody tr:nth-of-type(2) td:nth-of-type(2)'),
      getText('cohort2')
    );
    assert.equal(
      await getElementText('.programyear-list tbody tr:nth-of-type(3) td:nth-of-type(1)'),
      getText('2012 - 2013')
    );
    assert.equal(
      await getElementText('.programyear-list tbody tr:nth-of-type(3) td:nth-of-type(2)'),
      getText('cohort0')
    );
  });
  ...
  test('check competencies', async function (assert) {
    ...
    await visit(url);
    assert.equal(
      await getElementText(find(findAll('.programyear-list tbody tr:nth-of-type(1) td')[2])),
      5
    );
  });
  ...
  test('check objectives', async function (assert) {
    ...
    await visit(url);
    assert.equal(
      await getElementText(find(findAll('.programyear-list tbody tr:nth-of-type(1) td')[3])),
      5
    );
  });
  ...
  test('check directors', async function (assert) {
    ...
    await visit(url);
    assert.equal(
      await getElementText(find(findAll('.programyear-list tbody tr:nth-of-type(1) td')[4])),
      5
    );
  });
  ...
  test('check terms', async function (assert) {
    ...
    await visit(url);
    assert.equal(
      await getElementText(find(findAll('.programyear-list tbody tr:nth-of-type(1) td')[5])),
      5
    );
  });
  ...
  function getTableDataText(n, i, element = '') {
    return find(
      `.programyears .list tbody tr:nth-of-type(${n + 1}) td:nth-of-type(${i + 1}) ${element}`
    );
  }
